"""
Single-ended Growing String Method (SE-GSM) implementation.
Uses slightly different coordinates definitions than original and
does not relax with a double-ended method automatically
"""
import numpy as np
...
class SEGSM(Path):
    ...
    def _minimise_initial_point(self):
        """The initial point must be minimised without constraints"""
        QAOptimiser.optimise(
            self[0],
            maxiter=50,
            conv_tol="normal",
            method=self.method,
        ...
        self[0].print_xyz_file(filename="opt.xyz")
        return None

    def append(self, point) -> None:
        """
        Add a new point to the path, taking a step along the approximated
        reaction coordinate, and then optimise

        Args:
            point (Species): the new point
        """
        idx = len(self)
        constr = [self._get_driving_coordinate(point)]

        opt = QAOptimiser(maxiter=50, conv_tol="loose", extra_prims=constr)
        opt.run(
            point,
            method=self.method,
            n_cores=Config.n_cores,
    ...
    def _get_driving_coordinate(self, point: "Species"):
        ...
        coeffs = list(np.array(coeffs) / np.mean(coeffs))
