...
class deque_array {
    // A deque class of 1D arrays, and a maximum number of items,
    // acts similar to the Python deque
public:
    int maxlen;
    std::vector<arrx::array1d> data;
    std::vector<arrx::array1d*> ptrs;

    deque_array(int maxlen) : maxlen(maxlen) {
    ...
    void reset_ptrs() {
        // NOTE: According to C++ standard, pointers to vector items will be invalid
        // after any item allocation
        ptrs.clear();
        ptrs.reserve(this->size());
        for (size_t i = 0; i < this->size(); i++) {
            ptrs.push_back(&data[i]);
        }
    }

    void append(const arrx::array1d& arr) {
        // Add a new array to the deque
        if (this->size() < maxlen) {
            data.push_back(arr);
            this->reset_ptrs();
        } else {
            // deque already filled, so rotate the pointers
            std::rotate(ptrs.begin(), ptrs.begin() + 1, ptrs.end());
            // modify the last position
            arrx::noalias((*this)[this->size() - 1]) = arr;
        }
    }
